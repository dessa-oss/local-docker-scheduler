...
class TestScheduleJobs(unittest.TestCase):
    ...
    def _job_payload(self, job_bundle_name):
        ...
        return {
            ...
            'metadata': {'project_name': 'test', 'monitor_name': 'test_monitor', 'username': 'shaz'},
            'schedule': {
                'second': '*/2'
            },
            'gpu_spec': {}
    ...
    def test_submit_scheduled_job_creates_project_if_project_name_is_in_job_payload(self):
        job_bundle_0, _ = self._submit_and_schedule_job()
        job_payload = self._job_payload(job_bundle_0)

        job_project_name = job_payload['metadata']['project_name']
        projects_from_redis = self.redis.zrange('projects', 0, -1)

        self.assertIn(job_project_name.encode(), projects_from_redis)

    @unittest.skip("Skipped until pipeline green")
    def test_submit_scheduled_job_creates_monitor_name_to_job_entry_in_redis(self):
        import time

        job_bundle_0, _ = self._submit_and_schedule_job()
        job_payload = self._job_payload(job_bundle_0)

        job_project_name, job_monitor_name = job_payload['metadata']['project_name'], job_payload['metadata']['monitor_name']

        time.sleep(7)
        redis_key = f'projects:{job_project_name}:monitors:{job_monitor_name}:jobs'

        self.assertTrue(self.redis.exists(redis_key))
        self.assertGreaterEqual(2, len(self.redis.smembers(redis_key)))
